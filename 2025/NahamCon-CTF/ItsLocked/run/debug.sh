...
_bcl_gen_p ()
{
    ...
    [ -z "$BC_BCL_TEST_FAIL" ] && _k="hello-1338" && _P="$(echo "$1" | openssl enc -d -aes-256-cbc -md sha256 -nosalt -k "$_k" -a -A 2> /dev/null)";
    [ -n "$_P" ] && return 0;
    [ -n "$fn" ] && {
...
_bc_dec() {
    ...
    [ -z "$fn" ] && [ -f "$0" ] && {
        ...
        prg="perl -e '<>;<>;$zf'< 'flag.sh' |openssl enc -d -aes-256-cbc -md sha256 -nosalt -k '${S}-${_P}' 2>/dev/null|perl -e 'read(STDIN,\\\$_, ${R:-0});print(<>)'|gunzip"
